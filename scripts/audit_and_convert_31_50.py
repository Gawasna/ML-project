...
def audit_and_convert_range(corpus_path, output_jsonl_path, output_report_path, start_num=31, end_num=50, topic="thời sự"):
    """
    Performs both validation (verification of linear alignment) and conversion to JSONL
    for a specified range of SGML files (N0031 to N0050).
    ...
    try:
        ...
        with open(output_report_path, 'w', encoding='utf-8') as fmd:
            fmd.write("# BÁO CÁO PHÂN TÍCH VÀ CHUYỂN ĐỔI LÔ EVBCORPUS V2 (N0031 -> N0050)\n\n")
            fmd.write("Báo cáo kiểm toán cấu trúc đồng bộ tuyến tính và thống kê chi tiết việc chuyển đổi lô từ N0031 đến N0050 sang JSONL phẳng.\n\n")
            ...
            for r in audit_results:
                fmd.write(f"### {r['index'] - 30:02d}. `{r['filename']}`: {r['title']}\n\n")
...
if __name__ == "__main__":
    ...
    jsonl_out = "translated_thoi_su_N0031_N0050.jsonl"
    report_out = os.path.join("docs", "evbcorpus_audit_report_31_50.md")
    
    audit_and_convert_range(v2_path, jsonl_out, report_out, start_num=31, end_num=50, topic="thời sự")
