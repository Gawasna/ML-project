...
def audit_and_convert_range(corpus_path, output_jsonl_path, output_report_path, start_num=91, end_num=120, topic="thời sự"):
    """
    Performs both validation (verification of linear alignment) and conversion to JSONL
    for a specified range of SGML files (N0091 to N0120).
    ...
    try:
        ...
        with open(output_report_path, 'w', encoding='utf-8') as fmd:
            fmd.write("# BÁO CÁO PHÂN TÍCH VÀ CHUYỂN ĐỔI LÔ EVBCORPUS V2 (N0091 -> N0120)\n\n")
            fmd.write("Báo cáo kiểm toán cấu trúc đồng bộ tuyến tính và thống kê chi tiết việc chuyển đổi lô từ N0091 đến N0120 sang JSONL phẳng.\n\n")
            ...
            for r in audit_results:
                fmd.write(f"### {r['index'] - 90:02d}. `{r['filename']}`: {r['title']}\n\n")
...
if __name__ == "__main__":
    ...
    jsonl_out = "translated_thoi_su_N0091_N0120.jsonl"
    report_out = os.path.join("docs", "evbcorpus_audit_report_91_120.md")
    
    audit_and_convert_range(v2_path, jsonl_out, report_out, start_num=91, end_num=120, topic="thời sự")
